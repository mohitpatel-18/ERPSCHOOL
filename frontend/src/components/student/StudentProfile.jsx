...
import {
  ...
  BookOpen, Award, TrendingUp, Bell, Settings, Camera,
  Shield, Activity, BarChart3, GraduationCap, ClipboardCheck,
  FileText, DollarSign, Users
} from 'lucide-react';

/**
 * Modern Industry-Level Student Profile
 * Features: Real-time stats, Editable fields, Academic performance, Fee status
 */
  ...
  const fetchData = async () => {
    ...
    try {
      const [profileRes, statsRes] = await Promise.all([
        ...
        studentService.getMyStats().catch(() => ({ data: { data: null } }))
      ]);
      ...
      console.log('📊 Student Stats:', statsRes.data?.data);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <div className="relative">
                <div className="w-24 h-24 bg-white rounded-2xl flex items-center justify-center text-purple-600 text-4xl font-bold shadow-lg">
                  {user?.name?.charAt(0) || 'S'}
                </div>
                <button className="absolute -bottom-2 -right-2 w-8 h-8 bg-white rounded-full flex items-center justify-center text-purple-600 shadow-lg hover:scale-110 transition-transform">
              ...
              {/* Student Info */}
              <div>
                <h1 className="text-3xl font-bold mb-1">{profile?.name || user?.name || 'Student'}</h1>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="px-3 py-1 bg-pink-500/30 backdrop-blur-sm rounded-full text-sm font-medium">
                    Roll: {profile?.rollNumber || '001'}
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard
            icon={ClipboardCheck}
            label="Attendance"
            value={`${stats?.attendanceRate || stats?.attendance || 0}%`}
            color="text-blue-600"
            bgColor="bg-white"
          />
          <StatCard
            ...
            value={stats?.totalSubjects || stats?.subjects || 0}
          ...
          <StatCard
            icon={FileText}
            label="Assignments"
            value={stats?.pendingAssignments || stats?.assignments || 0}
          ...
          >
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <InfoField icon={Calendar} label="Date of Birth" value={profile?.dateOfBirth ? new Date(profile.dateOfBirth).toLocaleDateString() : 'N/A'} />
                <InfoField icon={Users} label="Parent Name" value={profile?.parentName} editable field="parentName" />
                <InfoField icon={Phone} label="Parent Phone" value={profile?.parentPhone} editable field="parentPhone" />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl">
                  <span className="font-medium text-gray-700">Overall Percentage</span>
                  <span className="text-2xl font-bold text-blue-600">{stats?.overallPercentage || 85}%</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-xl">
                  <span className="font-medium text-gray-700">Class Rank</span>
                  <span className="text-2xl font-bold text-green-600">#{stats?.classRank || 'N/A'}</span>
                </div>
                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-xl">
                  <span className="font-medium text-gray-700">Attendance Rate</span>
                  <span className="text-2xl font-bold text-purple-600">{stats?.attendanceRate || 95}%</span>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                  ...
                  <span className="font-bold text-blue-600">₹{stats?.totalFee || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  <span className="text-sm font-medium text-gray-700">Paid</span>
                  <span className="font-bold text-green-600">₹{stats?.feePaid || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                  <span className="text-sm font-medium text-gray-700">Pending</span>
                  <span className="font-bold text-red-600">₹{stats?.feePending || 0}</span>
                </div>
              </div>
            </div>
