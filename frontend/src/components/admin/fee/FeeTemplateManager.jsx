...
import { FaPlus, FaEdit, FaTrash, FaSave, FaTimes, FaEye, FaToggleOn, FaToggleOff } from 'react-icons/fa';
...
const FeeTemplateManager = () => {
  ...
  const handleToggleActive = async (template) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${API_URL}/fees/templates/${template._id}`,
      ...
      toast.error(error.response?.data?.message || `Failed to ${action} template`);
    }
  };
  ...
  const handleDelete = async (id) => {
    const confirmMessage = '⚠️ WARNING: This will permanently delete:\n\n' +
      '• The fee template\n' +
      '• All student fee records using this template\n' +
      '• All payment records related to those fees\n\n' +
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.delete(`${API_URL}/fees/templates/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (deletedData) {
        toast.success(
          `Template deleted! Removed ${deletedData.studentFees} student fees and ${deletedData.payments} payments.`,
          { duration: 5000 }
        );
      } else {
  ...
  const handleEdit = (template) => {
    ...
    setFormData({
      ...
      lastDate: template.lastDate ? template.lastDate.split('T')[0] : '',
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          title="Delete (Only if not assigned)"
                        >
                          <FaTrash />
                        </button>
                      </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full my-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Last Payment Date *</label>
                  <input
                    type="date"
